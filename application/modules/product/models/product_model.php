...
class Product_Model extends CI_Model
{
    ...
    //Get all products.
    public function feature($family)
    {
        ...
        $query .= " AND (product_category.family='".$family."' OR product_category.family='".str_replace("-", " ", $family)."')";
        $query .= " AND product_category.feature=1"; 
        ...
        if (count($data) == 0) {
            ...
            $query .= " WHERE product.display='Yes'";
            $query .= " AND (product_category.family='".$family."' OR product_category.family='".str_replace("-", " ", $family)."')";
            $query .= " GROUP BY product.id ORDER BY product.price ASC LIMIT 8";
...
//            }
            ...
            $msg = "(Metal price ".$settings['Metal prices'][$o->metal]." * metal weight ".$metalweight." * markup ".$settings['Markup'].") + initial price ".$o->price." + diamond price ".$dprice." = ".$price;
            comments_add($msg);
    ...
    public function PO4Price($o, $row = null) {
        ...
        //Dont allow user to buy if config doesn't show proper price.
       if($isDPPriceBAD || $isDCPriceBAD) {
           $mmprice = 0;
       }
        
        $msg = "Total = inital price ".$iprice." + metal price ".$mprice." + diamond parent ".$dpprice." + diamond child ".$dcprice." + fit price ".$fitprice." = ".$mmprice;
        comments_add($msg);
    ...
    {
        //echo $family." : ".$brand . " : " . $type;
        
        if($search != "") { 
            return array("Searched for '".ucwords($search)."'");
        } else if($brand == "all") { 
            return array("All ".ucwords($family));
            return array(ucwords($family));
        } else if($brand == "") { 
            return array("Featured ".ucwords($family));
        } else {
    ...
    //Get banner.
    //Process FBT then FB then F to see when a banner arises ... if any!
    public function banner($family = "", $brand = "", $type = "", $search = "") {
    
        $tbl = "productdesc";
        
        if($search == "") {
            $res1 = db_get_record($tbl, array("family" => $family, "brand" => $brand, "type" => $type));    
            $res2 = db_get_record($tbl, array("family" => $family, "brand" => $brand, "type" => ""));
            $res3 = db_get_record($tbl, array("family" => $family, "brand" => "", "type" => ""));        
...
//                echo "3";
            ...
            if($b == "") {
//                echo "4";
                $b = "banner_description_default.jpg";
            }
            
//            echo base_url('assets/img/'.$b);
            return base_url('assets/img/'.$b);
        }
        return base_url('assets/img/banner_description_default.jpg');
    ...
    public function get_found_price_range_message($products, $price_range) {
        ...
        if($num == 1) {
            ...
            $msg = "<span class=\"product-price\">".$num."</span> products have been found";
            if($price_range['min'] == 0 || $price_range['max'] == 0) {
                $msg .= ".";
            } elseif($price_range['min'] == 0) {
                $msg .= " and all cost <span class=\"product-price\">".fp($price_range['max'])."</span>";
            } elseif($price_range['max'] == 0) {
                $msg .= " and all cost <span class=\"product-price\">".fp($price_range['min'])."</span>";
            } elseif($price_range['min'] == $price_range['max']) {
                $msg .= " and all cost <span class=\"product-price\">".fp($price_range['min'])."</span>";
            } else {
                $msg .= " and range from <span class=\"product-price\">".fp($price_range['min'])."</span> to <span class=\"product-price\">".fp($price_range['max'])."</span>.";
            }
        }
                ...
                if ($family == "") {
                    ...
                    if ($brand == "") {
                        ...
                        if ($type == "") {                                  
                            ...
                            $metaTitleExtra = $family." - ".$brand;
                            $metaDescriptionExtra = "High quality ".$brand." ".$family;
                            $metaKeywordsExtra = ", ".$family.", ".$brand;

                        } else {

                            $metaTitleExtra = $family." - ".$brand." ".$type;
                            $metaDescriptionExtra = "High quality ".$type." ".$brand." ".$family; 
                            $metaKeywordsExtra = ", ".$family.", ".$brand.", ".$type; 
...
//        return $interest;
    ...
    public function LoadSettings($po) {
        ...
        $rows = $this->db->query("SELECT * FROM po3_metal order by sort_order")->result();
        foreach($rows as $row){
            if ($row->metal != "na") {
    ...
    public function getLeftMenuData($family) {
        
        $query  = "SELECT pc.family as family, pc.brand as brand, pc.type as type, count(pc.type) AS cnt FROM product p INNER JOIN product_category pc ON pc.id = p.id WHERE p.display='Yes' AND pc.family='".$family."' GROUP BY pc.family, pc.brand, pc.type";
        ...
        foreach($rows as $row){
            ...
            if(!isset($arr["brand"][$row->brand]["All"])) {
                $arr["brand"][$row->brand]["All"] = $row->cnt;
            } else {
                $arr["brand"][$row->brand]["All"] += $row->cnt;
            }           
            $type = ($row->type == "" ? "-" : $row->type);
            if(!isset($arr["brand"][$row->brand][$type])) {
                $arr["brand"][$row->brand][$type] = $row->cnt;
            } else {
                $arr["brand"][$row->brand][$type] += $row->cnt;
            } 
